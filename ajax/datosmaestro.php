...
$tara= isset($_POST['tara'])? limpiarCadena($_POST['tara']):'';
...
switch ($_GET["op"]){
    case 'guardaryeditar':
        if (empty($id)){
            ...
            $rspta=$datosmaestros->insertar($fechai,$horai,$contenedor,$barco,$tipocontenido,$descontenido,$detalleservicio,$destino,$idpiloto,$user_id,$observaciones, $tara);
          ...
          /*  $noposicionac= isset($_POST['noposicion']);
            ...
            $rspta=$datosmaestros->actualizar($id, $fechai,$horai,$contenedor,$barco,$tipocontenido,$descontenido,$detalleservicio,$marchamo,$horatir,$serietir,$producto,$orden,$bloque,$nposicion,$destino,$fechaasignacion,$idpiloto,$user_id,$observaciones);
              */
            $rspta=$datosmaestros->actualizar($id, $fechai,$horai,$contenedor,$barco,$tipocontenido,$descontenido,$detalleservicio,$destino,$idpiloto,$user_id,$observaciones,$tara);
        ...
        while ($reg=$rspta->fetch_object()){
            ...
            $datos[]=array(
               ...
               /* "3"=>$reg->Marchamo,
                ...
                "7"=>'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->Id_Ingreso.')"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="Editar Ingreso"></i></button>'.' '.$opcion
            );
        }
        $results=array(
            "sEcho"=>1,
